...
import { listBookingsForAgency } from '../services/bookings.service';
...
export default function AgencyDashboard() {
  ...
  const { data: agencyBookings, reload: reloadBookings } = useAsync(
    () => (agencyId ? listBookingsForAgency(agencyId) : Promise.resolve([])), [agencyId],
  );
  ...
  const pendingCount = (agencyBookings ?? []).filter((b) => b.status === 'requested').length;
    ...
    <PageTransition>
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
        <DashboardBanner
          ...
          eyebrow="Travel agency portal"
          title={currentUser?.businessName || 'Book boats for your clients'}
          icon={Briefcase}
        />

        <div className="mt-4"><VerificationBanner /></div>
        {!verified && <div className="mt-4"><DocumentUploader /></div>}

        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatTile index={0} icon={Users} label="Pending client requests" value={pendingCount} />
          <StatTile index={1} icon={Briefcase} label="Bookings made" value={(agencyBookings ?? []).length} />
                ...
                return (
                  <motion.div variants={staggerItem} key={boat.id} className="flex items-center gap-4 p-3 sm:p-4 transition-colors hover:bg-lake-50">
                    ...
                    <button
                      ...
                      disabled={!agencyId || !verified}
                      title={agencyId && verified ? '' : 'Your agency account must be verified before you can book for clients'}
        ...
        <div className="mt-10">
          <h2 className="font-semibold text-lake-950">Your client bookings</h2>
          <div className="mt-3 space-y-2">
            <AnimatePresence>
              {(agencyBookings ?? []).length === 0 && (
                <p className="text-sm text-lake-500">No bookings made yet.</p>
              )}
                ...
                <motion.div key={b.id} layout initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-lake-500">
                      ...
                      {b.captainName ? ` · Captain ${b.captainName}` : ''}
                    </p>
                  </div>
