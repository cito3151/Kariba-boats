...
const experienceOptions: (ExperienceType | 'all')[] = [
  'all',
  'fishing',
  'sunset',
  'houseboat',
...
export default function TouristHome() {
  const { boats } = useAppData();
  const [groupSize, setGroupSize] = useState(2);
  const [experience, setExperience] = useState<ExperienceType | 'all'>('all');
  ...
  const filtered = useMemo(() => {
    return boats.filter((b) => {
      if (experience !== 'all' && !b.experiences.includes(experience)) return false;
      if (b.capacity < Math.min(groupSize, 1)) return false;
      if (b.priceAmount > maxPrice) return false;
      if (verifiedOnly && !b.verified) return false;
      return true;
    });
  }, [boats, groupSize, experience, maxPrice, verifiedOnly]);

  const availableToday = boats.filter((b) => b.availableToday);
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              <div>
                ...
                <select
                  value={experience}
                  onChange={(e) => setExperience(e.target.value as ExperienceType | 'all')}
                ...
                >
                  ...
                  {experienceOptions.slice(1).map((e) => (
        ...
        <section className="mx-auto max-w-6xl px-4 sm:px-6 pt-12">
          <Reveal>
            <div className="flex items-center gap-2 mb-4">
              <Zap size={18} className="text-sunset-500" />
              <h2 className="font-display text-xl font-medium text-lake-950">Available today</h2>
              <span className="text-xs text-lake-500">Boats free right now for walk-in bookings</span>
            </div>
          </Reveal>
          <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1">
            {availableToday.map((boat) => (
              <div key={boat.id} className="min-w-[260px] max-w-[260px] shrink-0">
                <BoatCard boat={boat} />
              </div>
            ))}
          </div>
