-- Fix: migration 27 added `select ... from public.profiles where id = auth.uid()` inside
-- create_booking, but the function RETURNS TABLE(id uuid, ...), so a bare `id` is ambiguous
-- (42702). Qualify the profiles lookup with an alias. Body otherwise identical to migration 27.
...
begin
  ...
  select p.role::text, p.agency_id into v_role, v_caller_agency from public.profiles p where p.id = auth.uid();
